cat > /home/claude/insurance/report.js << 'ENDOFFILE'
// ================================================
// 보장분석 리포트 생성기 - 가온사업단 오피스 모듈
// v7.2: app-root 마운트 수정 + 탭 버튼 크기 고정 + 엑셀 패널 분리
// ================================================
...
const COVERAGE_RULES = [
  ...
  { key: 'death_injury',     kws: ['재해사망', '상해사망', '재해사망보험금', '평일일반재해사망'] },
  ...
  { key: 'car_injury',       kws: ['자동차사고부상치료비', '자동차부상치료지원금', '자동차사고 부상치료비', '자동차부상치료'] },
];
...
function extractInsurer(product) {
  ...
  const clean = product.replace(/\(무\)|\(무배당\)|무배당|무\)|해약환급금|미지급형|갱신형|\d+형/g, '').trim();
...
function parseInsuranceText(raw) {
  ...
  function emptyEntry(product) {
    ...
    return { displayName: product, insurer: extractInsurer(product), coverages: cov, premium: 0, start_date: '', end_date: '' };
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isProductLine(line)) {
      ...
      if (!productMap.has(nk)) { productMap.set(nk, emptyEntry(line)); productOrder.push(nk); }
      currentNormKey = nk;
      pendingLabel = null;
      continue;
    }
    if (AMT_RE.test(line)) {
      if (currentNormKey && pendingLabel) {
        ...
        if (key) { const amt = parseAmount(line); if (amt > 0) productMap.get(currentNormKey).coverages[key] += amt; }
      }
      pendingLabel = null;
      continue;
    }
    if (currentNormKey && line.length >= 2) pendingLabel = line;
  }

  const companies = [];
  for (const nk of productOrder) {
    const data = productMap.get(nk);
    companies.push({ name: data.insurer, product: data.displayName, start_date: data.start_date, end_date: data.end_date, premium: data.premium, coverages: data.coverages });
  }
...
// ─── ✅ [핵심수정] 마운트 대상을 'report-app' 또는 'app-root' 둘 다 지원 ───
window.initRptModule = function () {
  // report-app 또는 app-root 어느 쪽이든 찾아서 마운트
  const app = document.getElementById('report-app') || document.getElementById('app-root');
  if (!app || app.dataset.rptInited) return;
  app.dataset.rptInited = '1';
...
function getRptHTML() {
  ...
  <!-- ✅ [수정] 탭 버튼: flex:1 제거, auto width + 고정 padding으로 크기 통일 -->
  <div style="display:flex;gap:8px;margin-bottom:18px;flex-wrap:wrap;">
    <button id="rpt-mode-text-btn" class="rpt-mode-btn rpt-mode-btn-active" onclick="window.rptSwitchMode('text')">
    ...
    <div class="rpt-card">
      ...
      <div id="rpt-progress-wrap" style="display:none;margin-bottom:16px;">
        ...
        <div style="height:10px;background:#E5E8EB;border-radius:99px;overflow:hidden;">
          <div id="rpt-progress-bar" style="height:100%;width:0%;background:linear-gradient(90deg,#3182F6,#60A5FA);border-radius:99px;transition:width 0.45s cubic-bezier(0.4,0,0.2,1);"></div>
        </div>
    ...
    <div id="rpt-preview-panel" style="display:none;margin-top:0;">
      <div class="rpt-card" style="margin-bottom:0;border-top-left-radius:0;border-top-right-radius:0;border-top:none;">
        <p style="font-size:12px;color:#3182F6;margin:0 0 12px 0;font-weight:600;">
          💡 고객 이름, 금액, 텍스트를 클릭하면 직접 수정할 수 있습니다.
        </p>
...
/* ✅ [수정] 탭 버튼 — flex:1 제거, width:auto, 균일한 padding */
.rpt-mode-btn{
...
.rpt-mode-btn:hover{background:#E2E8F0;}
.rpt-mode-btn-active{background:#3182F6 !important;color:#fff !important;}
...
window.rptStartParsing = function () {
  ...
  if (text.trim().length < 10) { rptShowError('텍스트를 먼저 입력해주세요. (최소 10자 이상)'); return; }
  ...
  STEPS.forEach(({ delay, pct, text, status }) => setTimeout(() => setProgress(pct, text, status), delay));
    ...
    try {
      ...
      setTimeout(() => {
        if (step3) {
          ...
          if (summary) summary.textContent = `✅ 총 ${rptState.companies.length}개 보험 상품 분석 완료!`;
...
window.rptUpdateName = function (el) { rptState.customerName = el.innerText.trim(); };
window.rptUpdateText = function (el, idx, key) { rptState.companies[idx][key] = el.innerText.trim(); };
...
function renderPreview() {
  ...
  const N    = co.length;
  let html = `<table>`;
  html += `<tr class="r-title"><td colspan="${3 + N}"><span contenteditable="true" onblur="window.rptUpdateName(this)" style="border-bottom:1px dashed #A0AAB5;padding-bottom:2px;">${esc(name)}</span> 님 보장분석표</td></tr>`;
  ...
  co.forEach((c, i) => html += `<td class="r-date-alt" style="font-size:9px;" contenteditable="true" onblur="window.rptUpdateText(this,${i},'product')">${esc(c.product)}</td>`);
  html += `</tr>`;
  ...
  html += `<tr><th class="r-item r-fee" style="background:#D6DEE7;color:#001E42;">보험료</th><td class="r-sum r-fee" id="sum-premium">${fmtWon(totalPrem)}</td>`;
  ...
  COVERAGE_DEF.forEach((cov, idx) => {
    ...
    html += `<tr>`;
    if (cov.cat !== null) { const span = CAT_SPANS[cov.cat]?.rowspan || 1; html += `<td class="r-cat" rowspan="${span}">${cov.cat}</td>`; }
    html += `<td class="r-item">${cov.label}</td><td class="r-sum" id="sum-${cov.key}">${sum ? fmtMan(sum) : ''}</td>`;
    vals.forEach((v, i) => { html += `<td class="${isEven ? 'r-val-alt' : 'r-val'}" contenteditable="true" onblur="window.rptUpdateVal(this,${i},'${cov.key}')">${v ? fmtMan(v) : ''}</td>`; });
    html += `</tr>`;
  });
...
window.rptDownloadExcel = async function () {
  ...
  function mkCell(v, fill, fontColor, bold, align, fontSize, wrapText) {
    const s = { border:BD, fill: fill ? { type:'pattern', patternType:'solid', fgColor:{ rgb:fill } } : { type:'pattern', patternType:'none' }, font:{ name:'Malgun Gothic', sz:fontSize||10, bold:!!bold, color: fontColor ? { rgb:fontColor } : { rgb:'000000' } }, alignment:{ horizontal:align||'center', vertical:'center', wrapText:wrapText!==false } };
    if (v===null||v===undefined||v==='') return { v:'', t:'s', s };
    if (typeof v==='number') return { v, t:'n', s };
    return { v:String(v), t:'s', s };
  }
  function setCell(r,c,v,fill,fontColor,bold,align,fontSize,wrapText) { ws[XLSX.utils.encode_cell({r,c})] = mkCell(v,fill,fontColor,bold,align,fontSize,wrapText); }
  function addMerge(r1,c1,r2,c2) { merges.push({s:{r:r1,c:c1},e:{r:r2,c:c2}}); }
  let r=0;
  setCell(r,0,`${name} 님 보장분석표`,null,C.navy,true,'left',14,false);
  ws[XLSX.utils.encode_cell({r,c:0})].s.border = {...BD,bottom:{style:'medium',color:{rgb:C.navy}}};
  addMerge(r,0,r,2+N); r++;
  setCell(r,0,'주요\n보장',C.navy,C.white,true,'center',10); addMerge(r,0,r+4,0);
  setCell(r,2,'고객\n보장합산',C.gray,C.darkNav,true,'center',10); addMerge(r,2,r+3,2);
  setCell(r,1,'보험사',C.navy,C.white,true,'center',10);
  co.forEach((c,i) => setCell(r,3+i,c.name,C.gray,C.red,true,'center',10)); r++;
  setCell(r,1,'상품명',C.navy,C.white,true,'center',10); setCell(r,2,'',C.gray,null,false,'center');
  co.forEach((c,i) => setCell(r,3+i,c.product,C.white,'000000',false,'center',9)); r++;
  setCell(r,1,'가입시기',C.navy,C.white,true,'center',10); setCell(r,2,'',C.gray,null,false,'center');
  co.forEach((c,i) => setCell(r,3+i,c.start_date||'',C.gray,'000000',false,'center',9)); r++;
  setCell(r,1,'납입기간/\n만기시점',C.navy,C.white,true,'center',10); setCell(r,2,'',C.gray,null,false,'center');
  co.forEach((c,i) => setCell(r,3+i,c.end_date||'',C.white,'000000',false,'center',9)); r++;
  setCell(r,0,'',C.navy,C.white,true,'center'); setCell(r,1,'보험료',C.feeGray,C.darkNav,true,'center',10);
  const totalPrem = co.reduce((s,c)=>s+(c.premium||0),0);
  setCell(r,2,totalPrem||'',C.feeGray,C.darkNav,true,'right',10);
  co.forEach((c,i) => setCell(r,3+i,c.premium||'',C.feeGray,C.darkNav,true,'right',10)); r++;
  let catStartRow=-1, catCurr=null;
  COVERAGE_DEF.forEach((cov,idx) => {
    const isEven = idx%2===1, dataBg = isEven ? C.evenBg : C.oddBg;
    if (cov.cat!==null) { if (catCurr!==null) addMerge(catStartRow,0,r-1,0); catCurr=cov.cat; catStartRow=r; setCell(r,0,cov.cat,C.navy,C.white,true,'center',10); }
    ...
    vals.forEach((v,i) => setCell(r,3+i,v?fmtMan(v):'',dataBg,C.blue,false,'right',10,false)); r++;
  });
  if (catCurr!==null) addMerge(catStartRow,0,r-1,0);
  setCell(r,0,'* 본 자료는 단순 참고용이며 보험 보장에 대한 자세한 사항은 해당 증권과 약관을 참고하시기 바랍니다.',C.footBg,C.grayTxt,false,'left',9,false);
  addMerge(r,0,r,2+N);
  ws['!ref'] = XLSX.utils.encode_range({s:{r:0,c:0},e:{r,c:2+N}});
  ws['!merges'] = merges;
  ws['!cols'] = [{wch:5},{wch:22},{wch:12},...co.map(()=>({wch:12}))];
...
window.rptPrint = function () {
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>보장분석표 인쇄</title>
  <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700&display=swap" rel="stylesheet">
  <style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Noto Sans KR',sans-serif;background:#f0f0f0;padding:10px;}.print-wrapper{background:white;padding:8mm 6mm;width:fit-content;min-width:100%;}table{border-collapse:collapse;table-layout:auto;width:100%;}th,td{border:1px solid #C5CBD3;padding:3px 4px;text-align:center;font-size:9px;font-family:'Noto Sans KR',sans-serif;line-height:1.3;vertical-align:middle;white-space:nowrap;}.r-cat{background:#001E42!important;color:#fff;font-weight:700;width:22px;-webkit-print-color-adjust:exact;print-color-adjust:exact;}.r-item{background:#D6DEE7!important;font-weight:600;text-align:left;min-width:90px;-webkit-print-color-adjust:exact;print-color-adjust:exact;}.r-sum{background:#D6DEE7!important;font-weight:700;color:#001E42;min-width:55px;-webkit-print-color-adjust:exact;print-color-adjust:exact;}.r-hdr{background:#D6DEE7!important;font-weight:700;-webkit-print-color-adjust:exact;print-color-adjust:exact;}.r-ins{background:#D6DEE7!important;font-weight:700;color:#C00000;-webkit-print-color-adjust:exact;print-color-adjust:exact;}.r-date{background:#D6DEE7!important;-webkit-print-color-adjust:exact;print-color-adjust:exact;}.r-date-alt{background:#fff!important;font-size:8px;}.r-fee{background:#D7DDE4!important;font-weight:700;-webkit-print-color-adjust:exact;print-color-adjust:exact;}.r-val{background:#fff!important;color:#1E40AF;}.r-val-alt{background:#F8FAFC!important;color:#1E40AF;}.r-title td{font-size:13px;font-weight:700;text-align:left;padding:7px 8px;border-bottom:2px solid #001E42;}.r-foot td{background:#F8FAFC!important;font-size:8px;color:#64748B;text-align:left;padding:4px 6px;}@media print{@page{size:A4 landscape;margin:8mm 6mm;}html,body{background:white!important;padding:0;margin:0;}.print-wrapper{padding:0;}tr{page-break-inside:avoid;break-inside:avoid;}th,td{font-size:7.5px!important;padding:2px 3px!important;}[contenteditable]{outline:none!important;}}</style>
  </head><body><div class="print-wrapper">${content}</div><script>window.onload=function(){setTimeout(function(){window.focus();window.print();},800);};<\/script></body></html>`);
  win.document.close();
};
...
function fmtMan(v) {
  if (!v) return '';
  if (v>=10000) { const e=v/10000,r=Math.round(e*10)/10; return (Number.isInteger(r)?r:r.toFixed(1))+'억'; }
...
// ✅ [핵심수정] rptSwitchMode — 탭 active 클래스도 rpt-mode-btn-active 로 통일
window.rptSwitchMode = function (mode) {
  ...
  if (!textBtn || !excelBtn || !textPanel || !excelPanel) {
    console.warn('[rptSwitchMode] 요소를 찾을 수 없습니다:', { textBtn, excelBtn, textPanel, excelPanel });
    return;
  }

  if (mode === 'excel') {
    textBtn.classList.remove('rpt-mode-btn-active');
    excelBtn.classList.add('rpt-mode-btn-active');
    textPanel.style.display  = 'none';
    excelPanel.style.display = 'block';
    if (window.initRptExcelModule) window.initRptExcelModule();
  } else {
    excelBtn.classList.remove('rpt-mode-btn-active');
    textBtn.classList.add('rpt-mode-btn-active');
