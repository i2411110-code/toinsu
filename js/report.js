...
// ─── 텍스트 붙여넣기 파서용: 담보명(제안서 표기) → COVERAGE_DEF key 매핑 ───
const TEXT_LABEL_MAP = {
  ...
  '질병수술비': 'disease_surg',
  '상해수술비': 'injury_surg',
  '뇌혈관수술비': 'brain_heart_surg',
  '허혈심장질환수술비': 'brain_heart_surg',
  '질병종수술': 'type_surg',
  '상해종수술': 'type_surg',

  '질병일당': 'disease_hosp',
  '상해일당': 'injury_hosp',
  '1인실 입원일당': 'general_hosp',
  '1인실입원일당': 'general_hosp',

  '고액항암 치료비(표적)': 'targeted',
  '고액항암치료비(표적)': 'targeted',
  '중입자방사선치료비': 'chemo_rad',
  '암 주요치료비': 'cancer_main',
  '암주요치료비': 'cancer_main',

  '교통사고처리 지원금': 'car_settlement',
  '교통사고처리지원금': 'car_settlement',
  '변호사 선임비용': 'car_lawyer',
  '변호사선임비용': 'car_lawyer',
  '벌금': 'car_fine',
  '자동차사고 부상치료비': 'car_injury',
...
// 한글 금액("1억 300만원","100만원","5,000만원" 등) -> 만원 단위 정수
...
// ─── 텍스트 붙여넣기 파싱 ───
// 입력: 보장분석 제안서에서 복사한 평문 텍스트
// 출력: { companies: [...], customer_name: '' }  (Claude 분석 결과와 동일 구조)
function parsePastedText(raw) {
  const lines = raw.split(/\r?\n/).map(l => l.trim()).filter(l => l.length > 0);

  const companyMap = {}; // productName -> { name, product, coverages:{} }
  let i = 0;
  ...
  while (i < lines.length) {
    ...
    // 담보명 줄: 다음 줄이 충분/부족/미가입이면 담보명으로 간주
    if (i + 1 < lines.length && STATUS_WORDS.has(lines[i + 1])) {
      const covLabel = line;
      currentCovKey = TEXT_LABEL_MAP[covLabel] || null;
      i += 2; // 담보명, 상태 줄 건너뜀

      // 부족금액/권장금액/가입금액/미가입 라인들 건너뛰기
      while (i < lines.length) {
        const l = lines[i];
    ...
    // 상품/세부담보/금액 3줄 묶음 (현재 담보 항목에 속한 가입 내역)
    if (i + 2 < lines.length && /원$/.test(lines[i + 2])) {
      const productName = lines[i];
      const detailLabel = lines[i + 1];
      const amountStr   = lines[i + 2];
      const manwon = parseManwon(amountStr);

      if (currentCovKey) {
        if (!companyMap[productName]) {
          companyMap[productName] = { name: productName, product: detailLabel, premium: 0, start_date: '', end_date: '', coverages: {} };
        }
        const c = companyMap[productName];
    ...
    // 카테고리 헤더(사망, 암, 뇌혈관질환 등) 또는 인식 불가 줄 -> 건너뜀
    i += 1;
  }

  const companies = Object.values(companyMap);
  return { companies, customer_name: '' };
}
...
function getRptHTML() {
  ...
  <div class="rpt-card">
    ...
    <p class="rpt-step-desc">보장분석 제안서에서 복사한 텍스트를 그대로 붙여넣으면 AI 호출 없이 바로 엑셀로 정리합니다.</p>
    <textarea id="rpt-paste-text" class="form-control" placeholder="여기에 보장분석 제안서 텍스트를 붙여넣으세요"
      style="width:100%; height:160px; font-size:12px; line-height:1.5; resize:vertical; background:#F8FAFC; padding:10px; border:1px solid #E2E8F0; border-radius:8px;"></textarea>
    <div style="margin-top:8px; display:flex; align-items:center; gap:10px;">
      <label style="font-size:13px; color:#475569;">고객명:
        <input type="text" id="rpt-paste-customer-name" placeholder="고객" style="margin-left:6px; padding:6px 10px; border:1px solid #E2E8F0; border-radius:6px; font-size:13px; width:100px;">
      </label>
      <button class="btn-action" style="max-width:200px;" onclick="window.rptStartTextAnalysis()">
        <i class="bi bi-magic"></i> 텍스트로 분석
      </button>
    </div>
  </div>
