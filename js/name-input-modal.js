...
//
// users_portal/{email} 문서의 displayName 필드를 사용 — 기존 이메일 회원가입
// (handleAuthSubmit)과 동일한 저장 위치라서 메인 화면 등 기존 표시 로직과 바로 호환됨.
//
// 사용법 (js/app.js의 handleKakaoRedirectCallback 안, signInWithCustomToken 직후):
//
//   import { checkAndPromptUserName } from './name-input-modal.js';
//
//   const data = await res.json(); // { token, uid, email, displayName } - kakao-auth API 응답
...
// ── Firestore 연동 함수 ─────────────────────────────────────────────────
/**
 * users_portal/{email} 문서에 displayName 필드가 없으면 모달을 띄워 입력받고 저장한다.
 * 기존 이메일 회원가입(handleAuthSubmit)과 동일한 컬렉션/필드를 사용하므로,
 * 메인 화면·청구 화면 등에서 이미 쓰고 있는 window.currentUserDisplayName 로직과 그대로 호환된다.
 * 이미 이름이 있으면 아무 동작도 하지 않고 바로 반환한다.
 *
 * @param {import("https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js").Firestore} db
 * @param {string} email - 카카오 로그인 응답의 email (kakao-auth API가 반환)
 * @param {string} [kakaoDisplayName] - 카카오 닉네임, 입력창 기본값으로 사용
 * @returns {Promise<string|null>} 최종 이름 (이미 있었거나 새로 입력한 값), 이메일이 없으면 null
 */
export async function checkAndPromptUserName(db, email, kakaoDisplayName) {
  if (!email) {
    // 이메일 동의를 안 한 극소수 계정 등, email이 없으면 users_portal 키를 만들 수 없어 건너뜀
    console.warn('checkAndPromptUserName: email이 없어 이름 입력을 건너뜁니다.');
    return null;
  }

  const userRef = doc(db, 'users_portal', email);
  const snap = await getDoc(userRef);
  const existingName = snap.exists() ? snap.data().displayName : null;

  if (existingName) {
    window.currentUserDisplayName = existingName; // 기존 로직과 동일하게 전역 캐시에도 반영
  ...
  const name = await showNameInputModal({
    ...
    onSubmit: async (inputName) => {
      ...
      window.currentUserDisplayName = inputName;
    }
  });
